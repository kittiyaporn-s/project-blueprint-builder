  ...
  return (
    <div className="panel flex items-center justify-between gap-4 p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
      <div className="flex items-center gap-4">
        <span
          className={`grid size-14 place-items-center rounded-3xl bg-gradient-to-br ${toneClasses[tone]} text-white shadow-lg`}
        >
          <Icon className="size-7" />
        </span>
        <div>
          <div className="font-display text-4xl font-bold leading-tight text-blue-950">{value}</div>
...
function SkillGapPage() {
  ...
  const improvementPlans = [
    {
      title: "พัฒนาทักษะดิจิทัล (Digital Skills Training)",
      desc: "หลักสูตรด้าน Digital Literacy, Data Analysis, AI Tools",
      count: trainingEmployeeIds.size,
    },
    {
      title: "โครงการพี่เลี้ยง (Mentoring Program)",
      desc: "จับคู่พนักงานกับผู้เชี่ยวชาญในสายงาน",
      count: Math.max(1, Math.round(trainingEmployeeIds.size / 2)),
    },
    {
      title: "เวิร์กช็อปเชิงปฏิบัติการ (Workshop)",
      desc: "พัฒนาทักษะผ่านการเรียนรู้แบบลงมือทำ",
      count: Math.max(1, gapBySkill.length),
    },
    {
      title: "การเรียนรู้ออนไลน์ (e-Learning)",
      desc: "หลักสูตรตามสมรรถนะ เรียนได้ทุกที่ทุกเวลา",
      count: trainingEmployeeIds.size,
    },
  ];
  ...
  return (
    <AppShell title="Gap Analysis" description="วิเคราะห์ช่องว่างสมรรถนะ เพื่อระบุประเด็นการพัฒนา">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-slate-600">
                วิเคราะห์ช่องว่างสมรรถนะ เพื่อระบุประเด็นการพัฒนา
              </p>
      ...
      <section className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          icon={Users}
          label="พนักงานที่ต้องพัฒนา"
          value={trainingEmployeeIds.size}
          tone="warning"
        />
        <StatCard
          icon={Gauge}
          label="คะแนนความพร้อมรวม"
      ...
      <section className="mt-6 grid gap-5 xl:grid-cols-[1.15fr_0.85fr_0.85fr]">
        <div className="panel p-5">
          <h2 className="flex items-center gap-2 font-display text-xl font-bold text-blue-950">
            ...
            เปรียบเทียบระดับสมรรถนะ (Required vs Actual)
          </h2>
          <div className="mt-5 h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={gapBySkill.slice(0, 6).map((skill) => ({
                  name: skill.name.slice(0, 16),
                  required: 80,
                  actual: Math.max(30, 80 - skill.totalGap * 6),
                }))}
              >
                ...
                <Tooltip />
                <Bar dataKey="required" fill="#2563eb" radius={[8, 8, 0, 0]} />
                <Bar dataKey="actual" fill="#67d4f4" radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        <div className="panel p-5">
          ...
          <Table className="mt-3">
            <TableHeader>
              <TableRow>
                <TableHead>#</TableHead>
                <TableHead>สมรรถนะ</TableHead>
                <TableHead className="text-center">ช่องว่าง</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {gapBySkill.slice(0, 5).map((skill, index) => (
        ...
        <div className="panel p-5">
          ...
          <div className="mt-4 grid h-72 grid-cols-2 grid-rows-2 overflow-hidden rounded-2xl border border-blue-100 text-xs font-semibold">
            <div className="bg-emerald-50 p-3 text-emerald-700">
              พัฒนาต่อเนื่อง
              <br />
              Leadership
            </div>
            <div className="bg-rose-50 p-3 text-rose-700">
              เร่งด่วน
              <br />
              {gapBySkill[0]?.name ?? "Digital Literacy"}
            </div>
        ...
        <div className="panel p-5">
          ...
          <div className="mt-4 space-y-3">
            {improvementPlans.map((plan, index) => (
              <div
                key={plan.title}
                className="flex items-center gap-3 rounded-2xl border border-blue-100 bg-white/80 p-3 shadow-sm"
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate font-bold text-blue-950">{plan.title}</p>
                  <p className="truncate text-xs text-muted-foreground">{plan.desc}</p>
                </div>
                <span className="font-bold text-blue-700">{plan.count} คน</span>
              </div>
            ))}
          </div>
        </div>
        <div className="panel overflow-hidden p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="flex items-center gap-2 font-display text-xl font-bold text-blue-950">
              ...
              รายชื่อกลุ่มเป้าหมายที่ต้องพัฒนา
            </h2>
            <div className="flex flex-wrap gap-2">
              ...
              <div className="relative">
                ...
                <Input placeholder="ค้นหาชื่อพนักงาน..." className="w-[220px] bg-white/90 pl-9" />
              </div>
              <Button variant="link">ดูทั้งหมด</Button>
            </div>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-blue-50/80">
                  ...
                  <TableHead>สมรรถนะหลักที่มีช่องว่าง</TableHead>
                  <TableHead className="text-center">ระดับช่องว่าง</TableHead>
                  <TableHead>แผนการพัฒนา</TableHead>
                  <TableHead>สถานะ</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {gapByEmployee.slice(0, 6).map((employee, index) => (
                  <TableRow key={employee.id}>
                    ...
                    <TableCell>
                      {gapRows
                        .filter((row) => row.employee.id === employee.id)
                        .slice(0, 2)
                        .map((row) => row.skill.skill_name)
                        .join(", ")}
                    </TableCell>
                    <TableCell className="text-center">
                      <Badge className="rounded-full bg-rose-100 text-rose-700">
                    ...
                    <TableCell>
                      {improvementPlans[index % improvementPlans.length]?.title.split(" (")[0]}
                    </TableCell>
                    <TableCell>
